      ...
      <ul className="mt-12 grid gap-3 sm:grid-cols-2">
        {stacks.map((s) => (
          <li key={s.id}>
            <Link
              href={`/stacks/${s.slug}`}
              className="group flex h-full flex-col rounded-xl border border-line-2 bg-[--color-surface] p-6 transition-all hover:-translate-y-px hover:border-accent/30 hover:bg-accent/[0.04]"
            >
