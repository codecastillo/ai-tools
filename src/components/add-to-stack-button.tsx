...
export default function AddToStackButton({
  ...
  if (!mounted) {
    ...
    return (
      <span
        aria-hidden
        className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-line-2 bg-[--color-surface]/60 text-ink-faint opacity-0"
      />
    );
  }
    ...
    <button
      ...
      className={cn(
        ...
        inStack
          ...
          : 'border-line-2 bg-[--color-surface]/80 text-ink-faint backdrop-blur-sm hover:border-accent/40 hover:bg-accent/10 hover:text-accent-bright',
      )}
    >
      {inStack ? (
        <>
